...
Shader "Custom/IncWhiteNoise" {
	Properties {
		...
		_tmp ("Apply Buffer (9)", Int) = 0
		_Stage ("Stage", Float) = 2
		_numStage("Number of stages", Int) = 2
	...
	SubShader {
		...
		Pass{
			...
			//stage the whitenoise is at
			uniform float _Stage;
			uniform int _numStage;
			...
			vertexOutput vert (vertexInput v){
				...
				v.vertex.xyz += /*v.normal +*/ 
					sin((v.vertex.x + _Time * _PX* _RX) * _RX) *  _AX;
				v.vertex.xyz += /*v.normal +*/ 
					cos((v.vertex.y + _Time * _PY* _RY) * _RY) * _AY;
				v.vertex.xyz += /*v.normal +*/ 
					sin((v.vertex.z + _Time * _PZ* _RZ) * _RZ) * _AZ;
			...
			float4 frag(vertexOutput i) : COLOR
			{
				...
				//if the light is directional, the direction is just its position
				...
				float atten = lerp(1/distance, 1.0, _WorldSpaceLightPos0.w);
